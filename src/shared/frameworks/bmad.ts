...
function epicFromPath(filePath: string): string | null {
  const match = filePath.match(/\/epics\/([^/]+)\//)
  return match ? match[1] : null
}

function deriveStatus(stepsCompleted: string[], explicitStatus: string): FeatureStatus {
  if (explicitStatus) return explicitStatus as FeatureStatus
  return stepsCompleted.length === 0 ? 'backlog' : 'in-progress'
}

export function parseBmadFile(content: string, filePath: string): Feature {
  ...
  const stepsCompleted = getArrayValue('stepsCompleted')
  const explicitStatus = getValue('status')
  const epicInPath = epicFromPath(filePath)
  const epicInFm = getValue('epic')

  return {
    id: getValue('id') || defaultId,
    status: deriveStatus(stepsCompleted, explicitStatus),
    priority: (getValue('priority') as Priority) || 'medium',
    assignee: getValue('assignee') || null,
    epic: epicInPath || epicInFm || null,
...
export function serializeBmadFeature(feature: Feature, originalContent: string): string {
